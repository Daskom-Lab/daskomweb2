...
export default function Mandiri({ 
    ...
    const [userAnswers, setUserAnswers] = useState(() => {
        const savedAnswers = localStorage.getItem("mandiriAnswer");
        return savedAnswers ? JSON.parse(savedAnswers) : Array(questions.length).fill("");
    });
    
    const handleSubmit = () => {
        localStorage.setItem("mandiriAnswer", JSON.stringify(userAnswers)); 
        if (setAnswers) setAnswers(userAnswers);
        onNavigate("MandiriReview");
    };    
